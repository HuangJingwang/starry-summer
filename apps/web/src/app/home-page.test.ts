...
describe('home page', () => {
  ...
  test('renders the README screenshot portfolio hero instead of the immersive landing', () => {
    ...
    expect(source).toContain('<HomeIntroCard ownerName={profile.ownerName} />');
    ...
    expect(introSource).toContain('src="/images/aster-profile.png"');
    ...
    expect(source).not.toContain('在这里记录技术文章、工程实践和一些生活里的光。');
  ...
  test('keeps home card geometry consistent across day and night themes', () => {
    ...
    expect(readRule(css, '.portfolio-hero__content')).toContain('--reference-article-height: 160px;');
    expect(readRule(css, '.portfolio-hero__content')).toContain('--reference-nav-width: 280px;');
    expect(readRule(css, '.portfolio-hero__content')).toContain('--reference-nav-height: 434px;');
