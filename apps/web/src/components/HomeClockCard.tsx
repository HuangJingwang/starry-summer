...
function SevenSegmentDigit({ value }: { value: number }) {
  ...
  return (
    <svg className="portfolio-hero__clock-digit" width="29" height="52" viewBox="0 0 29 52" aria-hidden="true">
      {paths.map((path, index) => (
        <path
          key={path}
          className="portfolio-hero__clock-segment"
          d={path}
          data-active={segments[index] ? 'true' : undefined}
        />
      ))}
    </svg>
  );
